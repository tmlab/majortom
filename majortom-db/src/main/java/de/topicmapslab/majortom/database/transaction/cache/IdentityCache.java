...
public class IdentityCache implements IDataStore {
	...
	public void addSubjectLocator(ITopic t, ILocator locator) {
		...
		internalAddSubjectLocator(t, locator);
	}
	...
	public Set<ILocator> getSubjectLocators(ITopic t) {
		...
		if (!containsSubjectLocators(t)) {
			ITopic topic = createLazyStub(t);
			/*
			 * copy to internal store
			 */
			for (ILocator locator : (Collection<ILocator>) getTopicMapStore().doRead(t, TopicMapStoreParameterType.SUBJECT_LOCATOR)) {
				internalAddSubjectLocator(topic, locator);
			}
		}
	 ...
	 *            the subject-locator
	 */
	private void internalAddSubjectLocator(final ITopic t, final ILocator locator) {
		if (subjectLocators == null) {
	...
	public ITopic bySubjectLocator(ILocator l) {
		...
		ITopic topic = null;
		if (subjectLocators != null) {
			topic = subjectLocators.get(l);
		}
		if (topic == null) {
			topic = (ITopic) getTopicMapStore().doRead(getTopicMapStore().getTopicMap(), TopicMapStoreParameterType.BY_SUBJECT_LOCATOR, l);
	...
	public void addSubjectIdentifier(ITopic t, ILocator identifier) {
		...
		internalAddSubjectIdentifier(t, identifier);
	}
	 ...
	 */
	private void internalAddSubjectIdentifier(final ITopic t, final ILocator identifier) {
	...
	public Set<ILocator> getSubjectIdentifiers(ITopic t) {
		...
		if (!containsSubjectIdentifiers(t)) {
			 ...
			 * copy to internal store
			 */
			for (ILocator locator : (Collection<ILocator>) getTopicMapStore().doRead(t, TopicMapStoreParameterType.SUBJECT_IDENTIFIER)) {
				internalAddSubjectIdentifier(topic, locator);
			}
		}
	...
	public ITopic bySubjectIdentifier(ILocator l) {
		...
		ITopic topic = null;
		if (subjectIdentifiers != null) {
			topic = subjectIdentifiers.get(l);
		}
		if (topic == null) {
			topic = (ITopic) getTopicMapStore().doRead(getTopicMapStore().getTopicMap(), TopicMapStoreParameterType.BY_SUBJECT_IDENTIFER, l);
			if (topic != null && !isRemovedConstruct(topic)) {
				topic = createLazyStub(topic);
			} else {
				return null;
	...
	public void addItemIdentifer(IConstruct c, ILocator identifier) {
		...
		internalAddItemIdentifer(c, identifier);
		/*
	 ...
	 *            the item-identifier
	 */
	private void internalAddItemIdentifer(final IConstruct c, final ILocator identifier) {
	...
	public Set<ILocator> getItemIdentifiers(IConstruct c) {
		...
		if (!containsItemIdentifiers(c)) {
			 ...
			 * copy to internal store
			 */
			for (ILocator locator : (Collection<ILocator>) getTopicMapStore().doRead(c, TopicMapStoreParameterType.ITEM_IDENTIFIER)) {
				internalAddItemIdentifer(c, locator);
			}
		}
	...
	public IConstruct byItemIdentifier(ILocator l) {
		...
		if (itemIdentitiers != null) {
			construct = itemIdentitiers.get(l);
		}
		if (construct == null) {
			construct = (IConstruct) getTopicMapStore().doRead(getTopicMapStore().getTopicMap(), TopicMapStoreParameterType.BY_ITEM_IDENTIFER, l);
	...
	public IConstruct byId(String id) {
		...
		IConstruct construct = null;
		if (ids != null) {
			construct = (IConstruct) ids.get(id);
		}
		if (construct == null) {
			construct = (IConstruct) getTopicMapStore().doRead(getTopicMapStore().getTopicMap(), TopicMapStoreParameterType.BY_ID, id);
			if (construct != null && !isRemovedConstruct(construct)) {
				construct = createLazyStub(construct);
			} else {
				return null;
	...
	public <T extends IConstruct> T createLazyStub(T c) throws ConstructRemovedException {
		...
		if (!lazyStubs.containsKey(c.getId())) {
			T construct = LazyStubCreator.createLazyStub(c, getTransactionStore().getTransaction());
	...
	public IScope createLazyStub(IScope scope) throws ConstructRemovedException {
		...
		if (!lazyScopes.contains(scope)) {
			...
			return new ScopeImpl(scope.getId(), themes);
		}
		return scope;
	}
	...
	public Set<ITopic> getTopics() {
		for (ITopic topic : (Collection<ITopic>) getTopicMapStore().doRead(getTopicMapStore().getTopicMap(), TopicMapStoreParameterType.TOPIC)) {
			if (!isRemovedConstruct(topic)) {
		...
		for (IConstruct lazyStub : lazyStubs.values()) {
			if (lazyStub instanceof ITopic) {
				if (!isRemovedConstruct(lazyStub)) {
	...
	public ILocator createItemIdentifier(ITopicMap topicMap) {
		String itemIdentifier = topicMap.getLocator().getReference() + "/" + getTransactionStore().generateIdentity().getId();
...
// IConstruct c = (IConstruct)
// getTopicMapStore().doRead(getTopicMapStore().getTopicMap(),
...
// }
// if (constructItemIdentitiers == null ||
// !constructItemIdentitiers.containsKey(c)) {
...
// Set<ILocator> locators = (Set<ILocator>) getTopicMapStore().doRead(c,
...
// }
// if (topicSubjectIdentifiers == null ||
// !topicSubjectIdentifiers.containsKey(t)) {
...
// Set<ILocator> locators = (Set<ILocator>) getTopicMapStore().doRead(t,
...
// * the item-identifier
// */
// public void removeItemIdentifer(final IConstruct c, final ILocator
// identifier) {
// if (isRemovedConstruct(c)) {
// throw new TopicMapStoreException("Construct is already marked as removed!");
// }
...
// * the subject-identifier
// */
// public void removeSubjectIdentifier(final ITopic t, final ILocator
// identifier) {
// if (isRemovedConstruct(t)) {
// throw new TopicMapStoreException("Topic is already marked as removed!");
// }
...
// */
// if (constructItemIdentitiers != null &&
// constructItemIdentitiers.containsKey(t)) {
// for (ILocator l : constructItemIdentitiers.get(t)) {
// itemIdentitiers.remove(l);
// }
// constructItemIdentitiers.remove(t);
...
// */
// public ILocator createItemIdentifier(ITopicMap topicMap) {
// String itemIdentifier = topicMap.getLocator().getReference() + "/" +
...
// */
// public void replace(ITopic topic, ITopic replacement, IRevision revision) {
// Set<ILocator> itemIdentifiers =
// HashUtil.getHashSet(getItemIdentifiers(topic));
// Set<ILocator> subjectIdentifiers =
// HashUtil.getHashSet(getSubjectIdentifiers(topic));
// Set<ILocator> subjectLocators =
// HashUtil.getHashSet(getSubjectLocators(topic));
//
// /*
// * move all item-identifier
// */
...
// */
// public boolean containsIdentifier(ILocator locator) {
// return containsItemIdentifier(locator) || containsSubjectIdentifier(locator)
// || containsSubjectLocator(locator);
// }
//
// /**
// * Checks if the given locator is used as item-identifier.
...
// */
// @SuppressWarnings("unchecked")
// public <T extends IConstruct> T createLazyStub(T c) throws
// ConstructRemovedException {
// if (lazyStubs == null) {
// lazyStubs = HashUtil.getHashMap();
// }
// if (c == null) {
...
// }
// if (!lazyStubs.containsKey(c.getId())) {
// T construct = LazyStubCreator.createLazyStub(c,
// topicMapStore.getTransaction());
// lazyStubs.put(c.getId(), construct);
// return construct;
// }
// return (T) lazyStubs.get(c.getId());
