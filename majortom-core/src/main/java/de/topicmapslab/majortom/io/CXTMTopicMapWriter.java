...
public final class CXTMTopicMapWriter {
	 ...
	 * 
	 * @param out
	 *            The stream the CXTM is written onto.
	 * @param baseLocator
	 *            The base locator which is used to resolve IRIs against.
	 * @throws IOException
	 *             If an error occurs.
	 */
	public CXTMTopicMapWriter(OutputStream out, String baseLocator) throws IOException {
		...
		_construct2Id = HashUtil.getHashMap();
		_locator2Norm = HashUtil.getHashMap();
		_assoc2Roles = HashUtil.getHashMap();
		_topic2Roles = HashUtil.getHashMap();
	 ...
	 *             If an error occurs.
	 */
	public void write(TopicMap topicMap) throws Exception {
		removeDuplicates(topicMap);
	 ...
	 * 
	 * @param topicMap
	 *            The topic map from which the topic should be serialized.
	 * @param idx
	 *            A (upto date) type instance index.
	 * @return All topics which must be included into the output.
	 */
	private Topic[] _fetchTopics(final TopicMap topicMap, final TypeInstanceIndex idx) throws FeatureNotRecognizedException {
		Collection<Topic> types = idx.getTopicTypes();
		ITopicMap majorTM = null;
		if (topicMap instanceof ITopicMap) {
			majorTM = (ITopicMap) topicMap;
		}
		if ((types.isEmpty()) || ((majorTM != null) && (majorTM.getTopicMapSystem().getFeature(FeatureStrings.TOPIC_MAPS_TYPE_INSTANCE_ASSOCIATION)))) {
	 ...
	 * @param topics
	 *            A modifiable collection of topics.
	 * @param sid
	...
	private Topic _getTopicBySubjectIdentifier(TopicMap tm, Collection<Topic> topics, String sid) {
		Topic topic = tm.getTopicBySubjectIdentifier(tm.createLocator(sid));
		if (topic == null) {
			topic = new TypeInstanceTopic(tm.createLocator(sid));
			topics.add(topic);
		}
		return topic;
	}
	 ...
	 * 
	 * @param assoc
	 *            The association to retrieve the roles from.
	 ...
	 * 
	 * @param topic
	 *            The topic to retrieve the names from.
	 ...
	 * 
	 * @param name
	 *            The name to retrieve the variants from.
	 ...
	 * 
	 * @param topic
	 *            The topic to retrieve the occurrences from.
	...
	private void _writeTopic(final Topic topic) throws IOException {
		...
		List<Role> roles_ = HashUtil.getList(topic.getRolesPlayed());
		...
		for (int i = 0; i < roles.length; i++) {
			sb.append("association.").append(_indexOf(roles[i].getParent())).append(".role.").append(_indexOf(roles[i]));
			_attrs.clear();
	 ...
	 * @param pos
	 *            The position of the occurrence within the parent container.
	 * @throws IOException
	 *             If an error occurs.
	 */
	 ...
	 * @param pos
	 *            The position of the name within the parent container.
	 * @throws IOException
	 *             If an error occurs.
	 */
	 ...
	 * 
	 * @param tmo
	 *            The Topic Maps construct to take the item identifiers from.
	 * @throws IOException
	 *             If an error occurs.
	 */
	 ...
	 * 
	 * @param topic
	 *            The topic to which the reference should point to.
	 ...
	 *            The attributes.
	 * @param reifiable
	 *            The reifiable Topic Maps construct.
	 */
	...
	/**
	 * Canonical sort order: 1. [type] 2. [roles] 3. [scope] 4. [parent]
	 */
	...
	/**
	 * Canonical sort order: 1. [player] 2. [type] 3. [parent]
	 */
	...
	/**
	 * Canonical sort order: 1. [value] 2. [type] 3. [scope] 4. [parent]
	 */
	...
	/**
	 * Canonical sort order: 1. [value] 2. [datatype] 3. [scope] 4. [parent]
	 */
	...
	private final class RoleSetComparator extends AbstractSetComparator<Role> {
		...
		@Override
		int compareContent(Set<Role> o1, Set<Role> o2, int size) {
			int res = 0;
