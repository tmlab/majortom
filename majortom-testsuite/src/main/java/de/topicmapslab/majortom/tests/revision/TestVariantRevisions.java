...
public class TestVariantRevisions extends MaJorToMTestCase {
	...
	public void testRevision() throws Exception {
		...
		assertEquals(1, revision.getChangeset().size());
		assertEquals(v, revision.getChangeset().get(0).getOldValue());
		assertTrue(revision.getChangeset().get(0).getOldValue() instanceof ReadOnlyVariant);
		
		v = (IVariant) revision.getChangeset().get(0).getOldValue();
