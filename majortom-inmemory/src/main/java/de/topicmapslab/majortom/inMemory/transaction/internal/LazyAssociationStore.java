...
public class LazyAssociationStore extends AssociationStore {
	...
	public ITopic getPlayer(Role r) {
		...
		if (player == null) {
			player = getLazyIdentityStore().createLazyStub((ITopic) getStore().getRealStore().doRead((IAssociationRole) r, TopicMapStoreParamterType.PLAYER));
		}
		return player;
	}
	...
	public Set<IAssociationRole> getRoles(Association association) {
		...
		try {
			for (IAssociationRole role : (Set<IAssociationRole>) getStore().getRealStore().doRead((IAssociation) association, TopicMapStoreParamterType.ROLE)) {
	...
	public Set<IAssociationRole> getRoles(ITopic player) {
		...
		Set<IAssociationRole> roles = HashUtil.getHashSet();
		try {
			for (IAssociationRole role : (Set<IAssociationRole>) getStore().getRealStore().doRead(player, TopicMapStoreParamterType.ROLE)) {
				/*
				 * old player relation
				 */
