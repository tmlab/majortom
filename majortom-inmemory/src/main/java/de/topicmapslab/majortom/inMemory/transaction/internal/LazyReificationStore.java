...
public class LazyReificationStore extends ReificationStore {
	...
	public ITopic getReifier(IReifiable reifiable) {
		...
		if (reifier == null) {
			reifier = getLazyIdentityStore().createLazyStub((ITopic) getStore().getRealStore().doRead(reifiable, TopicMapStoreParamterType.REIFICATION));
		}
		return reifier;
	}
	...
	public IReifiable getReified(ITopic reifier) {
		...
		if (reifiable == null) {
			reifiable = getLazyIdentityStore().createLazyStub((IReifiable) getStore().getRealStore().doRead(reifier, TopicMapStoreParamterType.REIFICATION));
		}
		return reifiable;
	}
